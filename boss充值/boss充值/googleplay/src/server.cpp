...
int handle_init(int argc, char **argv, int pid_type)
{
    if (pid_type == PROC_WORK) {
        ...
        /*
        if (Init_SSL_CTX(IAP_SANDBOX) < 0) {
            DEBUG_LOG("Init_SSL_CTX sandbox Failed");
            return -1;
        }
        
        if (Init_SSL_CTX(IAP_PRODUCT) < 0) {
            DEBUG_LOG("Init_SSL_CTX sandbox Failed");
            return -1;
        }
        */
        ...
        char **send_buf, int *send_len, const skinfo_t *sk)
{
    ...
    *send_len = sizeof(proto_header_t);
    ...
    char *ack_body_buf = g_send_buf + sizeof(proto_header_t);
    int ack_body_len = 0;
    uint32_t *ack_status_code = &ack_header->status_code;

    switch (req_header->cmd_id) {
        case GOOGLEPLAY_PREORDER_CMD_ID:
            googleplay_preorder(user_id,
                    req_body_buf,
                    req_body_len,
                    ack_body_buf,
                    &ack_body_len,
        ...
        case GOOGLEPLAY_VERIFY_CMD_ID:
            googleplay_verify(user_id,
                    req_body_buf,
                    req_body_len,
                    ack_body_buf,
                    &ack_body_len,
    ...
    *send_len = ack_header->pkg_len = sizeof(proto_header_t) + ack_body_len;
