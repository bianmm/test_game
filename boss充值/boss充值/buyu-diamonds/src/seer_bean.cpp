 ...
 */
int sync_bean_balance(uint32_t user_id, uint32_t bean_num, uint32_t costs, uint32_t item_id, uint32_t channel_id)
{
	//-------------------
	//form package
...
int add_seer_bean(int channel_id,int account_id,int seer_bean_num, int costs, int item_id, int external_trans_id, uint32_t third_channel_id, uint64_t* p_transaction_id)
{
    //-------------------
    //param check
    ...
    try
    {
        ...
        else
        {
            //-------------------
            //update  seer_bean account
            sprintf(g_sz_query_sql,"UPDATE seer_bean_account_table SET costs = costs + %d WHERE account_id = %d", costs, account_id);
            if(mysql_real_query(p_db_connection,g_sz_query_sql,strlen(g_sz_query_sql)))
            {
                throw c_my_exception_t(mysql_errno(p_db_connection),mysql_error(p_db_connection));
            }
        ...
        sprintf(g_sz_query_sql,"INSERT INTO seer_bean_transaction_table(account_id,dest_account_id,product_id,channel_id,external_trans_id,seer_bean_num,seer_bean_balance) \
            VALUES(%d,%d,%u,%d,%d,%d,%d)",account_id,account_id,item_id,channel_id,external_trans_id,seer_bean_num,seer_bean_balance);
        ...
        {
            sync_bean_balance((uint32_t)account_id, (uint32_t)seer_bean_num, costs_balance, item_id, third_channel_id);
        }
        INFO_LOG("sync bean balance, account_id: %u, costs_balance: %u, item_id: %u", account_id, costs_balance, item_id);
...
extern "C" int handle_init(int argc,char* argv[],int pid_type)
{
    ...
    if(pid_type == PROC_WORK)
    {
        //-------------------
        //read config file
        const char* lpsz_config_file = "../etc/buyu_diamond.conf";
        ...
        g_sync_cmd_id = read_profile_int("sync_cmd","cmd_id",50151,lpsz_config_file);
...
extern "C" int handle_process(char* p_recv_buffer,int recv_len,char** pp_send_buffer, int* p_send_len, const skinfo_t* p_skinfo)
{
    ...
    else if(SEER_BEAN_PROTO_ADD_CMD_ID == cmd_id)
    {
        ...
        do
        {
            ...
            //-------------------
            //add  seer_bean
            proto_header.status_code = add_seer_bean(proto_body.verify_header.channel_id,proto_header.account_id,proto_body.seer_bean_num,  proto_body.costs, proto_body.item_id, proto_body.external_trans_id, proto_body.channel_id, &proto_ack.transaction_id);
