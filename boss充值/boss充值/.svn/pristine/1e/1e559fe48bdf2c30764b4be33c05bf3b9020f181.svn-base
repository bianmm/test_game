...
int handle_process(const char *recv_buf, int recv_len, char **send_buf,
	...
	*send_len = sizeof(proto_header_t);
    ...
    case ANYPAY_QUERY_CMD_ID:
        anypay_query(user_id, req_body_buf, req_body_len, rsp_body_buf,
                &rsp_body_len, ack_status_code);
            break;
	case ANYPAY_PREORDER_CMD_ID:
		anypay_preorder(user_id, req_body_buf, req_body_len, rsp_body_buf,
				&rsp_body_len, ack_status_code);
		break;
	case ANYPAY_NOTIFY_CMD_ID:
		anypay_notify(req_header->cmd_id, user_id, req_body_buf, req_body_len, rsp_body_buf,
				&rsp_body_len, ack_status_code);
		break;
    case PP_PAY_NOTIFY_CMD_ID:
        anypay_notify(req_header->cmd_id, user_id, req_body_buf, req_body_len, rsp_body_buf,
                      &rsp_body_len, ack_status_code);
        break;
    case XY_PAY_NOTIFY_CMD_ID:
        anypay_notify(req_header->cmd_id, user_id, req_body_buf, req_body_len, rsp_body_buf,
                          &rsp_body_len, ack_status_code);
        break;
    case BAIDU_SIGNE_NOTIFY_CMD_ID:
        anypay_notify(req_header->cmd_id, user_id, req_body_buf, req_body_len, rsp_body_buf,
                          &rsp_body_len, ack_status_code);
        break;
    case CHONGCHONG_PAY_NOTIFY_CMD_ID:
            anypay_notify(req_header->cmd_id, user_id, req_body_buf, req_body_len, rsp_body_buf,
                          &rsp_body_len, ack_status_code);
            break;
    case ANYPAY_NOTIFY_CLIENT_CMD_ID:
            anypay_notify_client(req_header->cmd_id, user_id, req_body_buf, req_body_len, rsp_body_buf, &rsp_body_len, ack_status_code);
            break;
	...
	*send_len = ack_header->pkg_len = sizeof(proto_header_t) + rsp_body_len;
    DEBUG_LOG("handle_process over, status_code: %d", *ack_status_code);
	return 0;
}
