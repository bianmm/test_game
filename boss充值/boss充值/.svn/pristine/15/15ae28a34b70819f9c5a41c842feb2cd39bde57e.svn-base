...
int c_mq_mgr_t::init(const char* p_mq_name,int mode, int msg_max, int msgsize_max)
{
    ...
    if(m_mq_fd < 0)
    {
    	ERROR_LOG("[errno:%d][errstr:%s]", errno, strerror(errno));
        return -1;
    }
 ...
 */
int c_mq_mgr_t::get_attr(long* mq_flags,long* mq_maxmsg,long* mq_msgsize,long* mq_curmsgs)
{
    //-------------------
    //check module status
    if(!m_inited)
	...
	{
		ERROR_LOG("get mq attr failed, errno: %d, errmsg: %s", errno, strerror(errno));
		return -1;
	}
