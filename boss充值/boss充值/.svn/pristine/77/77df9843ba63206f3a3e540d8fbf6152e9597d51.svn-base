...
class apple_iap_sys {
    ...
    ///////////////////////////////////////////
    const APPLE_PREORDER_CMD_ID = 0xA041;

    /** @brief 通过邮箱名得到米米号
      * @param email char[64]

      */
    public function apple_preorder($product_id, $ud) {
        $req_body = pack('A32A64', $product_id, $ud);
    ...
    ///////////////////////////////////////////
    const RECEIPT_VERIFY_CMD_ID = 0xA042;

    /** @brief 添加邮箱到米米号映射
      * @param email char[64]

      */
    public function receipt_verify($trans_id, $receipt) {
        $req_body = pack('Lsa'.strlen($receipt), $trans_id, strlen($receipt), $receipt);
        $req_header = pack('LLSLL', 18 + strlen($req_body), 0,
