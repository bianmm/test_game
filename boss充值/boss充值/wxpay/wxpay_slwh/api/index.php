...
$rst = json_decode(file_get_contents('http://192.168.1.21:8080/ipay/dowx?cmd=check_blh&blh='.$_REQUEST['inputName']));
if ($rst->ret != 1) {
    // 百乐号不存在
    $return_data['ret'] = 'E';
...
$mem->set($_REQUEST['openid'].'/uslwh', $_REQUEST['inputName']);
log_trace('set openid '.$_REQUEST['openid'].' -> uid '.$_REQUEST['inputName']);

// 2. 获得预交易订单号
$rst = json_decode(file_get_contents('http://192.168.1.21:8080/ipay/dowx?cmd=wxpay_preorder&pass=q35dgqtwas&blh='.$_REQUEST['inputName'].'&pdt_id='.$pdt_id.'&amt='.($_REQUEST['infullAmount'] * 100)));
